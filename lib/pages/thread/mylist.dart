...
/// desc:我的帖子列表
/// date:20190514
class MyThreadList extends StatefulWidget {
  @override
  createState() => MyThreadListState();
}

class MyThreadListState extends State<MyThreadList> {
  List<ThreadListItemInfo> list;
  int _curPage = 1;
  var _loadFailed = true;

  ///删除回调
  void _deleteCallBack(int index) {
    setState(() {
      list.removeAt(index);
    });
  }
  ...
  Future<List<ThreadListItemInfo>> loadList() async {
    var res = await ThreadDao.myList(page: _curPage);
    if (res == null) {
      setState(() {
        _loadFailed = true;
        return null;
